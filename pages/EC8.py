    ...
    border-width: 2px;}</style><div class="dashed-line"></div>""",
      ...
      **⁛** Η **μέγιστη τέμνουσα σχεδιασμού του υπσοτυλώματος** θα είναι: $$\text{{max}} \text{{V}}_{{\text{{d}}_\text{{(y)}}}} = {maxVd_y}\, \text{{kN}}$$ και η **ελάχιστη τέμνουσα σχεδιασμού του υποστυλώματος** θα είναι: $$\text{{min}} \text{{V}}_{{\text{{d}}_\text{{(y)}}}} = {- minVd_y}\, \text{{kN}}$$""")
      ...
      """)
            ...
            st.markdown(r"""
      $$\text{maxV}_{\text{d}_\text{(y)}} = \gamma_{\text{Rd}} \cdot \dfrac{\left[ \left( \text{M}_{\text{Rc,1}}^+ \cdot \dfrac{\sum \text{M}_{\text{Rb,άνω}}}{\sum \text{M}_{\text{Rc,άνω}}} \right) \right] + \left[ \left( \text{M}_{\text{Rc,2}}^- \cdot \dfrac{\sum \text{M}_{\text{Rb,κάτω}}}{\sum \text{M}_{\text{Rc,κάτω}}} \right) \right]}{\text{L}_{\text{cl}}}$$

      **και**

      $$\text{minV}_{\text{d}_\text{(y)}} = - \gamma_{\text{Rd}} \cdot \dfrac{\left[ \left( \text{M}_{\text{Rc,1}}^{-} \cdot \dfrac{\sum \text{M}_{\text{Rb,άνω}}}{\sum \text{M}_{\text{Rc,άνω}}} \right) + \left( \text{M}_{\text{Rc,2}}^{+} \cdot \dfrac{\sum \text{M}_{\text{Rb,κάτω}}}{\sum \text{M}_{\text{Rc,κάτω}}} \right)}{\text{L}_{\text{cl}}}$$
