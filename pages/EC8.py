            ...
            with col2_3:
                ...
                with col1:
                    ...
                    MRb_1a = st.number_input(
                        "**Αρνητική** ροπή αντοχής: $M_{Rb,1}^-$:", value=-150.0, step=0.1, format="%.2f"
                    )
                    MRb_1t = st.number_input(
                        "**Θετική** ροπή αντοχής: $M_{Rb,1}^+$:", value=300.0, step=0.1, format="%.2f"
                    )
                with col2:
                    st.success("**Δεξιά:**")
                    MRb_2a = st.number_input(
                        "**Αρνητική** ροπή αντοχής: $M_{Rb,2}^-$:", value=-200.0, step=0.1, format="%.2f"
                    )
                    MRb_2t = st.number_input(
                        "**Θετική** ροπή αντοχής: $M_{Rb,2}^+$:", value=400.0, step=0.1, format="%.2f"
                    )
            ...
            with col4:
                ...
                if de_dokos:
                    ...
                    MRb2r_a = st.number_input(
                        "**Αρνητική** ροπή αντοχής $M_{Rb,2_{right}}^-$:", value=-200.0, step=0.1, format="%.2f"
                    )
                    MRb2r_t = st.number_input(
                        "**Θετική** ροπή αντοχής $M_{Rb,2_{right}}^+$:", value=100.0, step=0.1, format="%.2f"
                    )
                else:
                    st.write("")
                ...
                if de_upost:
                    ...
                    if deupost_an:
                      ...
                      MRc_3a = st.number_input(
                          "**Αρνητική** ροπή αντοχής: $M_{Rc,3}^-$:", value=-350.0, step=0.1, format="%.2f"
                      )
                      MRc_3t = st.number_input(
                          "**Θετική** ροπή αντοχής: $M_{Rc,3}^+$:", value=700.0, step=0.1, format="%.2f"
                      )
                    else:
                        st.write("")
                    ...
                    if deupost_kat:
                      ...
                      MRc_4a = st.number_input(
                          "**Αρνητική** ροπή αντοχής: $M_{Rc,4}^-$:", value=-100.0, step=0.1, format="%.2f"
                      )
                      MRc_4t = st.number_input(
                          "**Θετική** ροπή αντοχής: $M_{Rc,4}^+$:", value=200.0, step=0.1, format="%.2f"
                      )
                    else:
                        st.write("")
                else:
  ...
  """)
    ...
    border-width: 2px;}</style><div class="dashed-line"></div>""",
        ...
        if SMRb_ar > SMRc_ar and SMRb_de > SMRc_de:
            ...
            st.markdown(r"""   
        $$\text{max} \text{V}_{\text{d}_\text{(x)}} = \dfrac{ \gamma_{\text{Rd}} \cdot \left[ \left( \text{M}_{\text{Rb,1}}^{-} \cdot \dfrac{\sum \text{M}_{\text{Rc,αρ}}}{\sum \text{M}_{\text{Rb,αρ}}} \right) + \left( \text{M}_{\text{Rb,2}}^{+} \cdot \dfrac{\sum \text{M}_{\text{Rc,δεξ}}}{\sum \text{M}_{\text{Rb,δεξ}}} \right) \right] }{\text{L}_{\text{cl}}} + \text{V}_{\text{G}+{\text{ψ}_2}{\text{Q}_o}_\text{(x)}}$$ 
        
        **και**
        
        $$\text{min} \text{V}_{\text{d}_\text{(x)}} = - \dfrac{ \gamma_{\text{Rd}} \cdot \left[ \left( \text{M}_{\text{Rb,1}}^{+} \cdot \dfrac{\sum \text{M}_{\text{Rc,αρ}}}{\sum \text{M}_{\text{Rb,αρ}}} \right) + \left( \text{M}_{\text{Rb,2}}^{-} \cdot \dfrac{\sum \text{M}_{\text{Rc,δεξ}}}{\sum \text{M}_{\text{Rb,δεξ}}} \right) \right] }{\text{L}_{\text{cl}}} + \text{V}_{\text{G}+{\text{ψ}_2}{\text{Q}_o}_\text{(x)}}$$ 
      ...
      ▧ Επειδή $$\sum \text{{M}}_{{\text{{Rb,αρ}}}} = {SMRb_ar}\, \text{{kNm}} > \sum \text{{M}}_{{\text{{Rc,αρ}}}} = {SMRc_ar}\, \text{{kNm και}} \sum \text{{M}}_{{\text{{Rb,δεξ}}}} = {SMRb_de}\, \text{{kNm}} \leq \sum \text{{M}}_{{\text{{Rc,δεξ}}}} = {SMRc_de}\, \text{{kNm}}$$""")
            ...
            st.markdown(r"""
      $$\text{max} \text{V}_{\text{d}_\text{(x)}} = \frac{ \gamma_{\text{Rd}} \cdot \left[ \left( \text{M}_{\text{Rb,1}}^{-} \cdot \frac{\sum \text{M}_{\text{Rc,αρ}}}{\sum \text{M}_{\text{Rb,αρ}}} \right) + \text{M}_{\text{Rb,2}}^{+} \right] }{\text{L}_{\text{cl}}} + \text{V}_{\text{G} + {\text{ψ}_2}{\text{Q}_o}_\text{(x)}}$$

      **και**

      $$\text{min} \text{V}_{\text{d}_\text{(x)}} = - \frac{ \gamma_{\text{Rd}} \cdot \left[ \left( \text{M}_{\text{Rb,1}}^{+} \cdot \frac{\sum \text{M}_{\text{Rc,αρ}}}{\sum \text{M}_{\text{Rb,αρ}}} \right) + \text{M}_{\text{Rb,2}}^{-} \right] }{\text{L}_{\text{cl}}} + \text{V}_{\text{G} + {\text{ψ}_2}{\text{Q}_o}_\text{(x)}}$$
      ...
      **Άρα:**   
      ...
      ▧ Επειδή $$\sum{{\text{{M}}}}_{{\text{{Rb,αρ}}}} = {SMRb_ar}\, \text{{kNm}} \leq \sum \text{{M}}_{{\text{{Rc,αρ}}}} = {SMRc_ar}\, \text{{kNm και}} \sum \text{{M}}_{{\text{{Rb,δεξ}}}} = {SMRb_de}\, \text{{kNm}} \leq \sum \text{{M}}_{{\text{{Rc,δεξ}}}} = {SMRc_de}\, \text{{kNm}}$$""")
            ...
            st.markdown(r"""
        $$\text{max} \text{V}_{\text{d}_\text{(x)}} = \gamma_{\text{Rd}} \cdot \frac{(\text{\text{M}}_{\text{Rb,1}}^{-} + \text{\text{M}}_{\text{Rb,2}}^{+})}{\text{L}_{\text{cl}}} + \text{V}_{\text{G}+{\text{ψ}_2}{\text{Q}_o}_\text{(x)}}$$

        **και**

        $$\text{min} \text{V}_{\text{d}_\text{(x)}} =  \gamma_{\text{Rd}} \cdot \frac{(\text{\text{M}}_{\text{Rb,1}}^{+} + \text{\text{M}}_{\text{Rb,2}}^{-})}{\text{L}_{\text{cl}}} + \text{V}_{\text{G}+{\text{ψ}_2}{\text{Q}_o}_\text{(x)}}$$
    ...
    """)
      ...
      """
        ...
        st.markdown(r"""
      $$\text{max} \text{V}_{\text{d}_\text{(y)}} =  \dfrac{\gamma_{\text{Rd}} \cdot \left[\text{M}_{\text{Rc,1}}^+ \cdot \min \left(1, \left(\frac{\sum \text{M}_{\text{Rb,άνω}}}{\sum \text{{M}}_{\text{Rc,άνω}}}\right)_1\right) + \text{{M}}_{\text{Rc,2}}^- \cdot \min \left(1, \left(\frac{\sum \text{{M}}_{\text{Rb,κάτω}}}{\sum \text{M}_{\text{Rc,κάτω}}}\right)_2\right)\right]}{\text{L}_{\text{cl}}}$$

        """)
        st.write("")
        st.markdown(r"""
      $$\text{min} \text{V}_{\text{d}_\text{(y)}} = - \dfrac{\gamma_{\text{Rd}} \cdot \left[\text{M}_{\text{Rc,1}}^- \cdot \min \left(1, \left(\frac{\sum \text{M}_{\text{Rb,άνω}}}{\sum \text{{M}}_{\text{Rc,άνω}}}\right)_1\right) + \text{{M}}_{\text{Rc,2}}^+ \cdot \min \left(1, \left(\frac{\sum \text{{M}}_{\text{Rb,κάτω}}}{\sum \text{M}_{\text{Rc,κάτω}}}\right)_2\right)\right]}{\text{L}_{\text{cl}}}$$""")
    ...
    border-width: 2px;}</style><div class="dashed-line"></div>""",
      ...
      **⁛** Η **μέγιστη τέμνουσα σχεδιασμού του υπσοτυλώματος** θα είναι: $$\text{{max}} \text{{V}}_{{\text{{d}}_\text{{(y)}}}} = {maxVd_y}\, \text{{kN}}$$ και η **ελάχιστη τέμνουσα σχεδιασμού του υποστυλώματος** θα είναι: $$\text{{min}} \text{{V}}_{{\text{{d}}_\text{{(y)}}}} = {- minVd_y}\, \text{{kN}}$$""")
      ...
      """)
            ...
            st.markdown(r"""
      $$\text{maxV}_{\text{d}_\text{(y)}} = \dfrac{\gamma_{\text{Rd}} \cdot \left( \text{M}_{\text{Rc,1}}^+ \cdot \dfrac{\sum \text{M}_{\text{Rb,άνω}}}{\sum \text{M}_{\text{Rc,άνω}}} \right) + \left( \text{M}_{\text{Rc,2}}^- \cdot \dfrac{\sum \text{M}_{\text{Rb,κάτω}}}{\sum \text{M}_{\text{Rc,κάτω}}} \right)}{\text{L}_{\text{cl}}}$$

      **και**

      $$\text{minV}_{\text{d}_\text{(y)}} = - \dfrac{\gamma_{\text{Rd}} \cdot \left( \text{M}_{\text{Rc,1}}^{-} \cdot \dfrac{\sum \text{M}_{\text{Rb,άνω}}}{\sum \text{M}_{\text{Rc,άνω}}} \right) + \left( \text{M}_{\text{Rc,2}}^{+} \cdot \dfrac{\sum \text{M}_{\text{Rb,κάτω}}}{\sum \text{M}_{\text{Rc,κάτω}}} \right)}{\text{L}_{\text{cl}}}$$
      ...
      **⁛** Η **μέγιστη τέμνουσα σχεδιασμού του υπσοτυλώματος** θα είναι: $$\text{{max}} \text{{V}}_{{\text{{d}}_\text{{(y)}}}} = {maxVd_y}\, \text{{kN}}$$ και η **ελάχιστη τέμνουσα σχεδιασμού του υποστυλώματος** θα είναι: $$\text{{min}} \text{{V}}_{{\text{{d}}_\text{{(y)}}}} = {minVd_y}\, \text{{kN}}$$""")
      ...
      """)
            ...
            st.markdown(r"""
      $$\text{maxV}_{\text{d}_\text{(y)}} = \gamma_{\text{Rd}} \cdot \dfrac{\left[\left( \text{M}_{\text{Rc,1}}^{+} \cdot \dfrac{\sum \text{M}_{\text{Rb,άνω}}}{\sum \text{M}_{\text{Rc,άνω}}} \right) + \text{M}_{\text{Rc,2}}^- \right] }{\text{L}_{\text{cl}}}$$

      **και**

      $$\text{minV}_{\text{d}_\text{(y)}} = - \gamma_{\text{\text{Rd}}} \cdot \dfrac{\left[\left \text{M}_{\text{Rc,1}}^{-} \cdot \dfrac{\sum \text{M}_{\text{Rb,άνω}}}{\sum \text{M}_{\text{Rc,άνω}}} \right) + \text{M}_{\text{Rc,2}}^+ \right] }{\text{L}_{\text{cl}}}$$
